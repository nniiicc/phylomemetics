...
/*
*   UGC - Comments
*   Original Author: Eddie Kennedy ( edward.kennedy@globe.com )
*   Additional Authors: Hugh Guiney ( hugh.guiney@globe.com )
*   Contents:
*   #Init #Forms #Actions #Comments 
*   #Util #Templates #Document Ready 
*/

var ugc = {}; // Define a global ugc for use across site
//var customEvents = customEvents || {};
//customEvents.commentsLoaded = new Event('commentsLoaded');
...
(function( win, $, undefined ) {
  ...
  */
  ugc.init = function() {
    if ( $('.comments').length ) { ugc.comments.init(); }
  };
  ...
  */
  ...
  ugc.comments = {
    ...
    init: function() {
      ...
      ugc.comments.pageType = 'comments';
      ugc.comments.sortOrder = ugc.util.getQueryStringValue('sort');
      ugc.comments.ia = ugc.util.getQueryStringValue("ia");
      ugc.comments.showReplies = ugc.util.getQueryStringValue("showReplies") ===  'true';
      ugc.comments.page = ugc.util.getQueryStringValue('page') || ugc.comments.page;
      ugc.comments.page = parseInt(ugc.comments.page, 10);

      if ( typeof topicId !== 'undefined' ) { ugc.comments.topicId = topicId; }
      if ( typeof uuid !== 'undefined' ) { ugc.comments.uuId = uuid; }
      if ( typeof pluckId !== 'undefined' ) { ugc.comments.pluckId = pluckId; }

      if ( ugc.comments.topicId ) {
        ugc.comments.fetchComments();
      } else if ( ugc.comments.pluckId || ugc.comments.uuId ) {
          var id = ugc.comments.pluckId || ugc.comments.uuId;
          ugc.comments.fetchTopicId(id).success(function(topicData) {
            if ( topicData.total > 0 ) {
              ugc.comments.topicId = topicData.topics[0].id;
              ugc.comments.fetchComments();
            } else {
              ugc.comments.createTopic(id).success(function(topicData) {
      ...
      //ugc.comments.initShowHideBar();
    },

    initShowHideBar: function () {
      //var $comments = ugc.comments.$root;
      var commentCount = ugc.comments.count || parseInt( ugc.comments.$root.attr('data-comment-count'), 10 ) || 0;
      var $commentCountDisplays = $('.js-comment-count');
      var $commentsToggleBar = $([
        '<a id="comments-toggle-bar" class="js-comments-toggle comments-toggle-bar" href="#comments">',
          '<span class="bg-icon-comment-light"></span> ',
          ( ( commentCount ? 'Show ' : '' ) + '<span class="js-comment-count">' + commentCount + '</span> ' ),
          ( '<span class="js-comment-count-label">' + pluralize( 'Comment', commentCount ) + '</span>' ),
        '</a>'
      ].join(''));

      function onCommentsToggleBarClick( event ) {
        event.preventDefault();
        var s;

        if ( ugc.comments.$root.hasClass("ugc-comments--active") ) {
          // Open -> Closed
          ugc.comments.$root.removeClass("ugc-comments--active");
          $commentsToggleBar.html( $commentsToggleBar.html().replace('Hide', 'Show') );

          // Analytics
          s = s_gi('nytbostonglobecom');
          s.tl(this,'o','Article_Comments_Hide');
        } else {
          // Closed -> Open
          ugc.comments.$root.addClass("ugc-comments--active");
          $commentsToggleBar.html( $commentsToggleBar.html().replace('Show', 'Hide') );
          
          // Analytics
          s = s_gi('nytbostonglobecom');
          s.tl(this,'o','Article_Comments_Expand');
        }
      }

      $commentCountDisplays.html( commentCount );
      
      if ( $('#comments-toggle-bar').length !== 1 ) {
        ugc.comments.$root.before( $commentsToggleBar );
      }

      $commentsToggleBar.on( 'click', onCommentsToggleBarClick );
    },

    // Event handlers
    eventHandlers: function() {
      // Show/hide replies
      $('.more-replies a').on('click', function(event) {
        event.preventDefault();
      ...
      //if ( $('.user_actions').length ) {
        ...
        $('.comment').on('click', '.user-action', function(event) {
          event.preventDefault();
    ...
    commentsUrl: function() {
      var url = '/ajax/comments.html?topic=' + ugc.comments.topicId;
      if ( ugc.comments.pageType === 'topic' ) { url += '&top=true'; }
      if ( ugc.comments.page ) { url += "&page=" + ugc.comments.page; }
      if ( ugc.comments.sortOrder ) { url += '&sort=' + ugc.comments.sortOrder; }
      if ( ugc.comments.ia ) { url += '&comment=' + ugc.comments.ia; }
      return url;
    },
    
    fetchComments: function() {
      var $root = ugc.comments.$root;

      return ugc.actions.ajaxCall( 'GET', ugc.comments.commentsUrl(), {}, true ).success(function (comments) {
        $root.replaceWith(comments); //.remove();

        $root = ugc.comments.$root = $('.ugc.comments-wrapper:eq(0)'); //$('#comments');
        //console.log( comments );

        //var commentsData = $( $(comments).filter('#comments').get(0) );

        //commentsData.filter

        //$root.html( commentsData.html() );

        // console.log( commentsData.html() );
        // console.log( commentsData.text() );
        // console.log( typeof comments );

        //$root.html(  );

        //gc.comments.$root = $('#comments');

        if (ugc.comments.showReplies) {
          ugc.comments.getReplies(ugc.comments.ia, $('#comment-' + ugc.comments.ia + ' .more-replies a'));
        }

        if (ugc.comments.ia) {
          ugc.util.goToEl( '#comment-' + ugc.comments.ia );
        }

        ugc.comments.eventHandlers();
        var commentData = $('.ugc.comments-wrapper').data();
        ugc.comments.count = commentData.commentCount;
        ugc.comments.updateCommentCounts();
        
        //if ( !ugc.comments.fetchCount ) {
          ugc.comments.fetchCount++;
          //console.log(ugc.comments.fetchCount);
          ugc.comments.initShowHideBar();
        //}
      });
    },
    ...
    submitComment: function( $form ) {
      ...
      if ( ugc.forms.checkForm( $form ) ) {
        ...
        ugc.actions.createIa( actionObj ).success(function(comment) {
          ...
          if ( ugc.comments.pageType === 'topic' ) {
            // Redirect to view all comments
            loc.href = "?comments=all&ia=" + comment.id;
          } else {
            // Refresh comments
            ugc.comments.fetchComments();
          }
    ...
    updateCommentCounts: function() {
      var $label = $('.js-comment-count-label');
      
      $('.total-comment-count, .js-comment-count').text( ugc.comments.count );
      
      $label.text( pluralize( 'Comment', ugc.comments.count ) );
    },
    ...
    submitReply: function( actionId ) {
      ...
      $formWrapper.find('form').on('submit', function(event) {
        ...
        ugc.actions.createIa( actionObj ).success(function() {
          ...
          if ( ugc.comments.pageType === 'topic' ) {
            // Redirect to view all comments
            loc.href = "?comments=all&ia=" + actionId;
          } else {
            // Refresh comments
            ugc.comments.getReplies( actionId, $('#comment-' + actionId + ' .more-replies a') );
            ugc.util.goToEl( '#comment-' + actionId );
            ugc.comments.count++;
            ugc.comments.updateCommentCounts();
          }
        });
    ...
    likeDislike: function( actionId, property, $el ) {
      ...
      ugc.actions.createIa( actionObj, true ).success( function() {
        // Update UI
        var $countSpan = $el.siblings('.count');
    ...
    getReplies: function( actionId, $el ) {
      var url = '/ajax/comments.html?parent=' + actionId;
      ...
      ugc.actions.ajaxCall( 'GET', url, {}, true ).success(function (replies) {
        if ( ugc.comments.pageType === 'topic' ) {
            // Append children
            var $childrenUl = $('<ul class="children" id="children-' + actionId + '" />').html(replies);
            $parent.find('.comment-holder').after($childrenUl);
    ...
    toggleReplies: function( $el ) {
      ...
      if ( elData.action === 'show' ) {
        ...
        if ( ugc.comments.pageType === 'topic' ) {
          $children.remove();
          $el.data({ action: 'show'}).text('Show replies (' + childCount + ')')
          .parent('div').removeClass('hide-more').addClass('show-more');
        } else {
          $children.children('li').filter(':gt(1)').hide();
          $el.data({ action: 'show'}).text('Show more replies (' + parseInt(childCount - 2, 10) + ')')
          .parent('div').removeClass('hide-more').addClass('show-more');
        }
      }
      ugc.util.goToEl( '#comment-' + elData.actionId );
    },
    ...
    adminAction: function( $el ) {
      ...
      $formWrapper.appendTo('body').find('form').submit(function(event) {
        ...
        $('.ugc-modal').remove();
        // Update UI
        $el.text( uiDataText ).attr({ 'data-value': uiDataValue });
      });
    }
  ...
  */
  ...
  ugc.util = {
    ...
    goToEl: function( elId ) {
      var $el = $(elId);
      if( $el.length ) {
        var elTop = $el.offset().top;
        $('html, body').animate( { scrollTop: elTop }, 500 );
        ugc.util.fadeColorOut( $el, '#ebebeb' );
      }
    },
  ...
  */
  ...
  ugc.templates = {
    // Comment reply form / returns $('<div class="comment new-reply">');
        ...
        '<div class="new-reply user-form">',
          '<form action="#" class="reply-form" method="post">',
            ...
            '<div class="form-input">',
              '<textarea type="text" id="reply-to-' + parent + '" class="add-reply-comment richtext" name="body" style="width:100%;" placeholder="Add a reply..."></textarea>',
            '</div>',
        ...
        '<div class="report-abuse user-form">',
          '<form action="#" class="report-abuse-form">',
            ...
            '<div class="form-section submit">',
              '<input type="submit" class="btn primary" value="Report" />',
              '<a href="#" class="btn cancel">Cancel</a>',
            '</div>',
          '</form>',
        '</div><!-- / .report-abuse -->'
...
(function(win, undefined){
    (function( $ ){
        ...
        $( function(){
            ...
            // Follow Nav (heading only)
            $.fn.scroller = function() {
                var $el      = this,
                    $par = this.closest( ".section-nav" ),
                    $ad      = $par.find( ".ad"),// offsets are based on this but now that's irrelevant...
                    $wrap    = $( "<div class='scroller-contain'></div>").prependTo($par),
                    $elH = $el.outerHeight(),
                    hedText  = $el.find( "h1 b" ).text(),
                    $jump    = $( "<a href='#section-nav'>" + hedText + "</a>" ).prependTo( $wrap ),
                //get wrap height, then add class to hide jump text
                    $wrapH   = $wrap.outerHeight(),
                    relC = "scroller-contain-relative",
                    fixC = "scroller-contain-fixed";
            ...
            // Section nav
            $.fn.followRail = function() {
                // How this works: Clone the rail, set the original to visibility hidden, steal the offsets from the original on resize

                //function get
                var $rail = this;
                var $railClone = $rail.clone();
                var $win = $( win );
                //var $par = this.closest( ".section-nav" );
                //var $ad = $par.find( ".ad"); // offsets are based on this but now that's irrelevant...
                //var $wrap = $( "<div class='scroller-contain'></div>").prependTo($par);
                var $railOffset = $rail.offset();
                var railHeight = $rail.outerHeight();
                var railWidth = $rail.outerWidth();
                
                var railPosRight = $railOffset.left + railWidth;
                var railPosFromRight = $win.width() - railPosRight;

                //console.log( $railOffset );

                //var hedText = $el.find( "h1 b" ).text();
                //var $jump = $( "<a href='#section-nav'>" + hedText + "</a>" ).prependTo( $wrap );
                // get wrap height, then add class to hide jump text
                //var $wrapHeight = $wrap.outerHeight();
                var railPosBottom = $railOffset.top + railHeight;
                var $articleContainer = $rail.parent('.article-container');
                var $articleBody = $articleContainer.find('.article-body:eq(0)');

                var articleBodyHeight = $articleBody.outerHeight();
                var articleBodyBottom = $articleBody.offset().top + articleBodyHeight;
                //var relativeClass = "scroller-contain-relative";
                //var fixedClass = "scroller-contain-fixed";

                $railClone.attr('id', $railClone.attr('id') + '-follow' ).addClass('nav-follow');

                $articleContainer.css('position', 'relative');
                $railClone.css('visibility', 'hidden');
                $rail.after( $railClone );

                //$jump.addClass( "ad-jump" );

                function resolvePositioning(){
                    var scrollPosTop = $win.scrollTop();
                    //var scrollPosRight = $win.width() - railPosRight;
                    ...
                    // If the viewport has enough room and the user has scrolled past
                    if (
                        ( $win.height() >= railHeight ) &&
                        ( railPosBottom < scrollPosTop ) &&
                        ( articleBodyBottom > scrollPosTop )
                    ) {
                        console.log('what');
                        $railClone.css({
                            "visibility": "visible",
                        ...
                        $railClone.css({
                            "visibility": "visible",
                            "position": "absolute",
                            //"top": ( articleBodyHeight + railHeight ),
                            "bottom": 0,
                            "right": 0
                        });
                        ...
                        console.log( 'articleBodyBottom', articleBodyBottom );
                        console.log( 'scrollPosTop', scrollPosTop );
                        console.log( 'railHeight', railHeight );
                ...
                function adjustPositioning() {
                    var newRailPosRight = $rail.offset().left + $rail.outerWidth();
                    var newRailPosFromRight = $win.width() - newRailPosRight;
                    // check media query???

                    $railClone.css({
                        "right": newRailPosFromRight
                    });
                }

                $win.on( "scroll load", resolvePositioning );
                $win.on( "throttledresize", adjustPositioning );
            };
            ...
            // Trending Articles
            var $sectionNav = $(".type-article .section-nav");
            var section_name = $sectionNav.attr("data-sectionname");
            var $navElement = $sectionNav.find('.bg-tabs-sidebar').parent();
            var $showTabLinks = $('.js-show-tab');

            $showTabLinks.on('click', function onTabClick( event ) {
              event.preventDefault();

              var $tab = $(this);
              var showTargetHref = $tab.attr('href');
              var $showTarget = $(showTargetHref);
              var $allTargets = $tab.closest('nav').siblings('.aside');

              $allTargets.attr('hidden', 'hidden');
              $showTabLinks.removeClass('here');

              $showTarget.removeAttr('hidden');
              $tab.addClass('here');
            });

            // /SysConfig/WebPortal/BostonGlobe/Framework/skins/leaf/story/most_emailed_right.jpt
            // /SysConfig/WebPortal/BostonGlobe/Framework/skins/leaf/story/most_popular_right.jpt
            // /SysConfig/WebPortal/BostonGlobe/Framework/skins/leaf/story/most_viewed_right.jpt
            // /SysConfig/WebPortal/BostonGlobe/Framework/skins/leaf/story/most_comment_right.jpt

            if ( $(".type-article .section-nav").length ) {
               $.when(
                    $.get("/Fragment/SysConfig/WebPortal/BostonGlobe/Framework/skins/leaf/story/most_viewed_right.jpt"),
                    $.get("/Fragment/SysConfig/WebPortal/BostonGlobe/Framework/skins/leaf/story/most_popular_right.jpt"),
                    $.get("/Fragment/SysConfig/WebPortal/BostonGlobe/Framework/skins/leaf/story/most_comment_right.jpt")
                ).done(function (mostViewed, mostPopular, mostCommented) {
                    // Each argument is an array with the following structure: [ data, statusText, jqXHR ]
                    $navElement.after(mostViewed[0]);
                    $navElement.after(mostPopular[0]);
                    $navElement.after(mostCommented[0]);
                    $showTabLinks.filter('.here').trigger('click');
                });
            }

            //inject print links here? save links? font sizer?
            /* Commented out for BGLOBE-3071. The necessary part is just below this comment
             * var videoDiv = $("#video");
            ...
            //The following two lines are adding omniture tagging to the print element in the share tools.
            //Top
            $(".primary-tools:nth-of-type(1) .share-print a").attr({ "onClick":"var s=s_gi('nytbostonglobecom');s.tl(this,'o','ArticleTools_Print_Top')",
                "href":"javascript:print();"
            });
            //Bottom
            $(".primary-tools:nth-of-type(2) .share-print a").attr({ "onClick":"var s=s_gi('nytbostonglobecom');s.tl(this,'o','ArticleTools_Print_Bottom')",
                "href":"javascript:print();"
            });
            // In this section
            var isClick = false;
            ...
            // Stub view?
            var isStub = $('body').hasClass('stub');

            if ( !isStub ){
                if( globe.loggedIn || isClick ){
                    var loadPreviews = function() {
                        var section    = $('h1.section').text(),
                            prettySection = $('#masthead h1.section').text();

                        if (!prettySection.length) {
                           var prettySection = "this section";
                        }

                           var  sContainer = [
                                '<div class="in-section">',
                                    '<h1 class="hed-section">More in <a href="' + section_name +'">' + prettySection + ':' + '</a></h1>',
                                '</div>'
                            ].join("");
                        ...
                        if ($(window).scrollTop() >= ($(document).height() - $(window).height() - 1000)) {
                            if (!$("article.article .in-section").length) {
                                $("article.article").append(sContainer);
                                $.get("/Fragment/SysConfig/WebPortal/BostonGlobe/Framework/skins/leaf/story/in_section_bottom.jpt?section=" + section_name, function(data) {
                                        $("article.article .in-section").append(data).filterHeds();
                                });
                            }
                        }
                    }
            ...
            $('ul.tools').each(function(){
                var tools = $(this);
                var email = tools.find('li.email a.email');
                var emailForm = tools.find('li.email div.email-form');
                var emailFormInput = tools.find('li.email div.email-form input');
                var classes = {
                    email : "email-on"
                }
...
//                    });
                ...
                email
                    .click(function(){
                        tools.toggleClass(classes.email);
                        //emailForm.focus();
                        $('#email-to-primary',email.parent()).focus();
                        return false;
                    })
//                    .focusout(function(){
//                        tools.toggleClass(classes.email);
...
//                    $(window).not(emailFormInput).on('click', function () {
//                        tools.removeClass(classes.email);
//                    })
...
//			// Email form and Share menu toggles
//			$('ul.tools').each(function(){
//				var email = $(this).find('li.share-button.email'),
...
//			});
            ...
            $(document).bind("click touchend", function(e) {
                $("li.share-button .email, li.share")
                    .not( $(e.target).closest("li") )
                    .removeClass("email-on share-on");
            });

            $("ul.tools ul a").bind("touchend", function(e){
            ...
            //mbox trigger from MEC for Test and Target
            //mboxCreate('MEC_ArtStub');
        });//domready
...
(function(win, $, undefined){
    ...
    function moveLeft() {
        $('.feat-strip ul li:last-child').prependTo('.feat-strip ul');
        module.$featStrip.css('margin-left', - module.widthOfArticle);
        module.$featStrip.animate({ "margin-left": 0 }, 300);
    }
    function moveRight() {
        module.$featStrip.animate({
            "margin-left": - module.widthOfArticle
        },300, function () {
            $('.feat-strip ul li:first-child').appendTo('.feat-strip ul');
            module.$featStrip.css('margin-left', '');
        });
    }

    module.tabletInit = function() {
        $('.feat-strip-wrapper').css('overflow-x','auto');
        $('.feat-button-left, .feat-button-right').css('display','none');
    };

    module.desktopInit = function() {
    ...
    // Kickoff
    $(document).ready(function() {
        if ($('.feat-strip').length > 0) {
            module.$leftButton = $('.feat-button-left');
            module.$rightButton = $('.feat-button-right');
            module.$featStrip = $('.feat-strip ul');
            module.widthOfArticle = $('.feat-strip-article').outerWidth();
...
//                   if (adKeys[DFPPosition].sizeByBreakpoint) {
//
//
//
...
//
//                   }
                   if (adKeys[DFPPosition].outOfPage) {
                    // it is an oop ad
                         googletag.defineOutOfPageSlot("/" + networkCode + "/" + globe.dfp.adUnit , [adWidth,adHeight],  DFPPosition)
                           .addService(googletag.pubads())
